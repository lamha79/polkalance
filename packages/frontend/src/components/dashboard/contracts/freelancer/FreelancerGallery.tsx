...
const FreelancerGallery: FC = () => {
  ...
  const { setSignAndObtainModalOpen, setJobIdForForm, jobIdForForm, submitModalOpen, useFormDone, setUseFormDone, type} = useLanding();
  //////
  ...
  const searchJobs = async () => {
    ...
    try {
      const result = await contractQuery(api, activeAccount.address ,contract, 'get_all_jobs_of_freelancer_with_status', {}, ['becreatingcontract']);
      const { output, isError, decodedOutput } = decodeOutput(result, contract, 'get_all_jobs_of_freelancer_with_status');
      const json = JSON.stringify(output, null, 2);
      const list_jobs = JSON.parse(json);
      const data = list_jobs.Ok;
